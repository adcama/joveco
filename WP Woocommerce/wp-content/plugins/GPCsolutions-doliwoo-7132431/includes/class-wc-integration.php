...
if ( ! class_exists( 'Doliwoo_WC_Integration' ) ) :
	...
	class Doliwoo_WC_Integration extends WC_Integration {
		...
		public function validate_dolibarr_ws_endpoint_field( $key ) {
			...
			// Validacion de HTTPS deshabilitada por hack
			...
			try {
				...
				new SoapClient( $value . Doliwoo_Dolibarr::OTHER_ENDPOINT . Doliwoo_Dolibarr::WSDL_MODE, array('stream_context' => $context) );
			} catch ( SoapFault $exc ) {
		...
		private function test_webservice(
			...
			try {
				...
				$soap_client = new SoapClient( $webservice . Doliwoo_Dolibarr::OTHER_ENDPOINT . Doliwoo_Dolibarr::WSDL_MODE, array('stream_context' => $context) );
			} catch ( SoapFault $exc ) {
				$this->errors[] = __( 'The webservice is not available. Please check the URL.', 'doliwoo' );
				$this->display_errors();
