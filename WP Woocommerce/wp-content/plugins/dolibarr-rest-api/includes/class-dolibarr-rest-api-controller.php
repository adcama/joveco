...
/**
 * Dolibarr REST API Controller class
 */
class Dolibarr_REST_API_Controller extends Dolibarr_REST_API {

	/**
	 * The single instance of Dolibarr_REST_API_Controller.
	 *
	 * @var    object
	 * @access private
	 * @since  1.0.0
	...
	/**
	 * Main Dolibarr_REST_API_Controller Instance
	 *
	 * Ensures only one instance of Dolibarr_REST_API_Controller is loaded or can be loaded.
	 *
	 * @since 1.0.0
	 * @static
	 ...
	 * @param string $file    File pathname.
	 * @param string $version Version number.
	 * @return Dolibarr_REST_API_Controller instance
	 */
	...
	/**
	 * Check connectivity to Dolibarr EventsManager module API
	 *
	 * @access  public
	 * @since   1.0.0
	 *
	...
	public function check_connectivity() {
		...
		if ( ! defined( 'DOLIBARR_REST_API_URL' )
			|| ! defined( 'DOLIBARR_REST_API_USER_KEY' ) ) {

			$this->add_error( __( 'Please define the DOLIBARR_REST_API_URL & DOLIBARR_REST_API_USER_KEY constants in the wp-config.php file.', 'dolibarr-rest-api' ) );
		...
		if ( ! filter_var( DOLIBARR_REST_API_URL, FILTER_VALIDATE_URL ) ) {

			$this->add_error( __( 'Please enter a valid DOLIBARR_REST_API_URL in the wp-config.php file.', 'dolibarr-rest-api' ) );
		...
		$response = $this->get( '/status', true );

		if ( empty( $response ) || is_wp_error( $response ) ) {

			$this->add_error( __( 'Please enter a valid DOLIBARR_REST_API_URL and DOLIBARR_REST_API_USER_KEY in the wp-config.php file.', 'dolibarr-rest-api' ) );

			// Reintent live (no cache) as we just had an error!
			$this->delete_cached_request( 'get', '/status' );
		...
		if ( 200 === $this->status &&
			...
			// Dolibarr API module error:
			// "Module Api must be enabled. To activate modules, go on setup Area (Home->Setup->Modules)."
		...
		if ( true === $response ) {
			...
			$this->add_error( __( 'Connection successful.', 'dolibarr-rest-api' ), 'success' );
		}
	...
	public function endpoint() {
		...
		$this->endpoint = rtrim( DOLIBARR_REST_API_URL, '/' ) . '/api/index.php/';
	...
	public function get_action_url( $action ) {
		...
		return $this->endpoint() . ltrim( $action, '/' );
	}
	 ...
	 * @return string
	 */
	public function dolapikey_header() {

		if ( isset( $this->dolapikey_header ) ) {

			return $this->dolapikey_header;
		}

		$this->dolapikey_header = array( 'DOLAPIKEY' => DOLIBARR_REST_API_USER_KEY );
	...
	public function get_request_args( $args = array() ) {
		...
		if ( empty( $this->request_args['headers'] ) ) {
			...
			$this->request_args['headers'] = $this->dolapikey_header();
		}
	...
	public function handle_error( $wp_error ) {
		...
		if ( is_wp_error( $wp_error ) ) {
			...
			$error_message = __( 'Empty response. Unknown error.', 'dolibarr-rest-api' );
		}
	...
	public function handle_body( $response ) {
		...
		if ( ! empty( $this->body ) ) {
			...
			if ( ! empty( $this->body->error->message ) ) {

				$message = $this->body->error->code . ' ' . $this->body->error->message;

				if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {

					$message .= ' &mdash; ' . $response['http_response']->get_response_object()->url;

					$message .= '<br /><pre>' . var_export( $this->body->debug, true ) . '</pre>';
				}
	...
	public function add_error( $message, $level = 'error', $plugin = 'this' ) {
		...
		if ( 'this' === $plugin ) {
			...
			$plugin = __( 'Dolibarr REST API', 'dolibarr-rest-api' );
		}
	...
	public function save_errors_to_transient( $post_id = 0 ) {
		...
		$transient_name = 'dolibarr_rest_api_errors_' . $post_id;
	...
	public function cache_request( $method, $action, $response, $data = array() ) {
		...
		$transient_name = 'dolibarr_rest_api_cache_' . $method . '_' . $action;
